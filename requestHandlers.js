...
module.exports = {
    stockAmountRequestHandler: async function (req, res) {
        if (!req.query.stock || !req.query.goal) {
            ...
            await fetch(getCompanyInfoURL).then(async (res) => {
                let returns = res.body.pipeThrough(new TextDecoderStream()).getReader();
                let arr = mock_IBM_data;
            ...
            console.log(bob);
            res.status(200).send({ ...bob, amountToBuy: calculateStockAmount(bob, goal).toFixed(2) });
